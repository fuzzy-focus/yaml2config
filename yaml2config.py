...
@click.option('--out-dir', '-d', default='.',
              type=click.Path(exists=True, writable=True, resolve_path=True),
              help='write output to this directory (default is .)')
@click.option('--template-dir', '-t', default='./template',
              type=click.Path(exists=True, readable=True, resolve_path=True),
              help='template search path (default is ./template)')
@click.option('--update-templates', '-u', is_flag=True,
              help='if template folder is git repo, pull from origin:master')
...
def _load_yaml(yaml_file):
    ...
    try:
        ...
        click.echo(click.style('Error in config file:' + str(yaml_file),
                               fg='red') + '\n' + repr(e), err=True)
        sys.exit(1)
    return config_data
...
def _load_template(template_dir, template_file):
    ...
    try:
        ...
        click.echo(click.style('Error in the template file:'
                   + str(Path(template_dir) / template_file), fg='red')
                   + '\n' + repr(e), err=True)
...
def _write_config(config_data, template, out_file):
    ...
    try:
        ...
        click.echo(click.style('Error during rendering:' + str(out_file),
                               fg='red') + '\n' + repr(e), err=True)
        raise WriteConfigError()
    else:
        with open(out_file, 'w') as f:
            f.write(rendered)
        click.echo('created config file at ' + click.style(str(out_file),
                   fg='yellow'))
...
def _update_templates_repo(template_dir):
    ...
    try:
        ...
        click.echo(click.style('GitPython is not available or not configured.',
                   fg='red') + '\n' + repr(e), err=True)
        sys.exit(1)
    try:
        ...
        click.echo(click.style('No valid repository at ' + str(template_dir),
                   fg='red') + '\n' + repr(e), err=True)
        sys.exit(1)
